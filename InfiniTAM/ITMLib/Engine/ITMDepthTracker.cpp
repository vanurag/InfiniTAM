...
ITMDepthTracker::ITMDepthTracker(Vector2i imgSize, TrackerIterationType *trackingRegime, int noHierarchyLevels, int noICPRunTillLevel, float distThresh,
    ...
    : pc_viewer("ICP visualizer"), scene_cloud_pointer(new pcl::PointCloud<pcl::PointXYZRGB>),
      current_view_cloud_pointer(new pcl::PointCloud<pcl::PointXYZRGB>)
{
...
void ITMDepthTracker::TrackCamera(ITMTrackingState *trackingState, const ITMView *view)
{
	...
	if (view->view_type == ITMView::ODOM) {
    ...
    double dist = sqrt((new_state-init_state).transpose()*odom_cov.inverse()*(new_state-init_state));
    std::cout << "Outlier distance: " << dist << std::endl;
    if (dist > 25.0) { // outlier
      trackingState->pose_d->SetM(initPose);
      gp_outlier_dist.push_back(std::make_pair(sdkGetTimerValue(&(trackingState->tracking_timer)), 0.0));
    } else {
      gp_outlier_dist.push_back(std::make_pair(sdkGetTimerValue(&(trackingState->tracking_timer)), dist));
    }
...
void ITMDepthTracker::visualizeTracker(
  ...
  // current view
  FloatImagetoPclPointCloud(current_view, current_view_cloud_pointer, intrinsics,
...
//  pc_viewer.removeAllPointClouds();
  ...
  // matches
  DrawPointMatches(current_view_cloud_pointer, matches, Vector3i(255, 255, 255));
