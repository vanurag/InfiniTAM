  ...
  const CONSTPTR(Vector4f) *normalsMap, float distThresh)
{
  ...
  tmp3Dpoint.x = depthPoint.x;
  tmp3Dpoint.y = depthPoint.y;
  ...
  const CONSTPTR(Vector4f) *normalsMap, float distThresh)
{
  ...
  Vector4f match = computePerPointGH_Depth_Ab<shortIteration,rotationOnly>(A, b, x, y, depthPoint, viewImageSize, viewIntrinsics, sceneImageSize, sceneIntrinsics, approxInvPose, scenePose, pointsMap, normalsMap, distThresh);
